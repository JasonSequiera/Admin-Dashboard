...
const Dashboard = () => {
  ...
  const updateRemarks = async (id, alreadyResolved = false) => {
  ...
  const responseRate =
    totalQueries > 0
      ? ((resolvedCount / totalQueries) * 100).toFixed(0)
      : 0;
    ...
    <div className="min-h-screen bg-gradient-to-br from-background via-background to-card">
      ...
      <header className="border-b border-border bg-card/50 backdrop-blur-sm sticky top-0 z-10">
        <div className="container mx-auto px-6 py-4 flex justify-between items-center">
          <div className="flex items-center space-x-4">
            ...
            <h1 className="text-2xl font-bold gradient-text select-none cursor-default">
              Admin Dashboard
            </h1>
          </div>
      ...
      <main className="container mx-auto px-6 py-8">
        ...
        <Card className="neon-border bg-card/80 backdrop-blur-sm">
          <CardHeader>
            <CardTitle className="text-xl gradient-text select-none cursor-default">
              User Queries
            </CardTitle>
          </CardHeader>
          <CardContent>
                ...
                <Table>
                  ...
                  <TableBody>
                    {queries.map((query, index) => (
                      <TableRow
                        key={query.id}
                        className="border-border hover:bg-muted/30 transition-colors select-none"
                      >
                        ...
                        <TableCell>
                          {editingId === query.id ? (
                            <div className="flex space-x-2">
                              <input
                                type="text"
                                className="px-2 py-1 border rounded text-sm text-black select-text"
                              ...
                              <Button
                                size="sm"
                                onClick={() =>
                                  updateRemarks(query.id, query.is_resolved)
                                }
                              >
                                Save
                              </Button>
                        ...
                        <TableCell className="flex items-center space-x-2">
                          {!query.is_resolved ? (
                            ...
                            <>
                              <span className="cursor-default text-green-500 font-medium">
                                Resolved
                              </span>
                              <Pencil
                                className="w-4 h-4 text-muted-foreground cursor-pointer hover:text-neon-blue"
